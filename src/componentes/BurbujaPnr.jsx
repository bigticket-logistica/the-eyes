...
// ═══════════════════════════════════════════════════════════════════════════
// BURBUJA DE CHAT · canal de Posventa.
//
// Copia de Burbuja.jsx con un solo cambio: el bucket. Se duplicó en vez de
// parametrizar el original porque ese archivo lo usan Consultas y la Ticketera,
// que atienden a los conductores en ruta, y no vale tocarlo para estrenar otro
// canal.
//
// ADJUNTOS: el bucket pnr-pruebas es PRIVADO, así que no sirve una URL fija.
// Se pide una URL firmada al abrir el hilo. La firma se cachea en memoria
// (55 min) para no re-firmar en cada refresco ni en cada render.
//
// El binario lo baja a Storage el worker pnr-media.cjs del VPS de Posventa.
// Si media_path está vacío, el adjunto aún no se procesó: se muestra el estado
// en lugar de un recuadro roto — un media_id de Meta caduca en cinco minutos y
// hay casos en que la descarga no llega a tiempo.
// ═══════════════════════════════════════════════════════════════════════════
...
async function urlFirmada(path) {
  ...
  const { data, error } = await sb.storage.from("pnr-pruebas").createSignedUrl(path, 3600);
